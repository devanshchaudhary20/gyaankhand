...
Subcommands:
  ...
  publish         Read state/_pending.json, post to Instagram as a 2-slide carousel,
                  record to state/posted.json.
  all             Render + publish in one go (useful for local testing).
...
def render_step(dry_run: bool = False) -> dict:
    ...
    base_image = verse_loader.pick_base_image(verse)

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    image_relpath = f"posts/{timestamp}-{verse['id']}.jpg"
    image_path = config.ROOT / image_relpath

    print(f"[gyaankhand] verse:  {verse['id']} ({verse['source']})")
    print(f"[gyaankhand] base:   {base_image.name}")
    print(f"[gyaankhand] output: {image_path}")
...
def publish_step(info: dict | None = None) -> str:
    ...
    image_url = instagram_poster.public_media_url(info["image_relpath"])
    print(f"[gyaankhand] image url: {image_url}")

    media_id = instagram_poster.post_carousel(image_url=image_url, caption=info["caption"])
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    sub.add_parser("publish", help="Publish previously-rendered image as Instagram carousel.")
